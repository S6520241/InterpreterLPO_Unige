...
import finalProject.parser.ast.*;

import static java.util.Objects.requireNonNull;
import static finalProject.parser.TokenType.*;
...
public class Parser implements ParserInterface {
	...
	private WhileStmt parseWhileStmt() throws ParserException {
		consume(WHILE); // or tokenizer.next() since WHILE has already been recognized
		consume(OPEN_PAR);
	...
	/*
	 * parses expressions, starting from the lowest precedence operator IN which is
	 * left-associative
	 * IsIn ::= DiffUnion (IN DiffUnion)*
	 */
	...
	/*
	 * parses expressions, starting from the lowest precedence operator SetOp which is
	 * left-associative
	 * DiffUnion ::= Add (SetOp Add)*
	 */
	private Exp parseDiffUnion() throws ParserException {
		...
		while (tokenizer.tokenType() == DIFF | tokenizer.tokenType() == UNION) {
			final var tokenType = tokenizer.tokenType();
			tokenizer.next();
			exp = switch (tokenType) {
				case DIFF -> exp = new Diff(exp, parseAdd());
				default -> new Union(exp, parseAdd());
			};
		}
		return exp;
	}
	 ...
	 * Atom ::= FST Atom | SND Atom | MINUS Atom | NOT Atom | SIZE Atom | BOOL | NUM | IDENT | OPEN_PAR Exp CLOSE_PAR
	 */
	...
	/*
	 * parses expressions of type SetAtom
	 * SetAtom ::= OPEN_BLOCK (FOR IDENT IN Exp EXP_SEP)? Exp CLOSE_BLOCK
	 */
	private Exp parseSetAtom() throws ParserException {
		...
		if (tokenizer.tokenType() == FOR) {
			...
			final var var = parseVariable();
			consume(IN);
			final var setExp = parseExp();
			consume(EXP_SEP);
			final var elemExp = parseExp();
			consume(CLOSE_BLOCK);
			return new SetEnum(var, setExp, elemExp);
		} else {
			final var exp = parseExp();
			consume(CLOSE_BLOCK);
			return new SetLit(exp);
		}
	}
	...
	private Size parseSize() throws ParserException {
		consume(SIZE); // can be omitted if the method is only called by parseAtom()
